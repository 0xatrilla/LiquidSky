...
public final class Auth: @unchecked Sendable {
    ...
    public func logout() async throws {
        ...
        // Also clear any session-related data that might persist
        UserDefaults.standard.removeObject(forKey: "Auth.lastSessionCheck")
        UserDefaults.standard.removeObject(forKey: "Auth.sessionData")
    }
    ...
    private func resetInternalState() {
        ...
        self.configuration = nil
            ...
            print(
                "Auth: After reset - account handles: \(accountManager.accounts.map { $0.handle })")
        #endif
    }
    ...
    public init(accountManager: AccountManager) {
            ...
            let activeAccount = accountManager.accounts.first(where: { $0.id == activeAccountId })
        {
            ...
            #if DEBUG
                print(
                    "Auth: No active account, using first available account: \(firstAccount.handle)"
                )
            #endif
    ...
    public func addAccount(handle: String, appPassword: String) async throws -> Account {
        ...
        // Always check if we're in fresh login state from a previous logout
        let freshLoginState = UserDefaults.standard.bool(forKey: "Auth.isInFreshLoginState")
        ...
        if freshLoginState {
            ...
            if let existingAccount = accountManager.accounts.first(where: { $0.handle == handle }) {
                if existingAccount.isActive {
                    ...
                    #endif
                    // Instead of throwing error, let's clear the account and allow fresh login
                    accountManager.clearAllAccounts()
                    currentAccountId = nil
                    #if DEBUG
                        print("Auth: Cleared existing accounts to allow fresh login")
                    #endif
                } else {
                    #if DEBUG
                        print(
                            "Auth: Found inactive account with handle \(handle), will reactivate it"
                        )
                    #endif
        ...
        // Authenticate with the new account (retry on transient conflicts)
        do {
            #if DEBUG
                print("Auth: Attempting authentication...")
            #endif
            try await runWithRetry(maxRetries: 3, initialDelaySeconds: 0.25) {
        ...
        #if DEBUG
            print(
                "Auth: Got user session for handle: \(session.handle), DID: \(session.sessionDID)")
        #endif
    ...
    public func restoreSession() async {
        do {
            ...
            guard
                let activeAccount = accountManager.accounts.first(where: {
...
extension Auth {
    fileprivate func runWithRetry<T>(maxRetries: Int = 3, initialDelaySeconds: Double = 0.2, operation: () async throws -> T) async throws -> T {
        var lastError: Error?
        for attempt in 0..<maxRetries {
            do {
                ...
                let delay = initialDelaySeconds * pow(2, Double(attempt))
                let ns = UInt64(delay * 1_000_000_000)
                try? await Task.sleep(nanoseconds: ns)
            }
        }
        throw lastError ?? AuthError.authenticationFailed
    }
}
...
public enum AuthError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .loginConflict:
            return "Temporary login issue detected. Please try again."
        }
    }
