    ...
    public var body: some View {
        feedListView
            ...
            .onAppear {
                ...
                feedPositionService.restoreSavedPosition()
            }
            .onDisappear {
                                ...
                                PostRowView(post: postGroup.posts.first!)
                                    ...
                                    .onDisappear {
                                        ...
                                        visiblePosts.removeAll {
                                            $0.uri == postGroup.posts.first!.uri
                                        }
                                    }
                            }
                        }
                    ...
                    VStack(spacing: 0) {
                        ...
                        .overlay {
                            Circle()
                                .stroke(
                                    post.hasReply
                                        ? LinearGradient.avatarBorderReversed
                                        : LinearGradient.avatarBorder,
                                    lineWidth: 1)
                        }
                    ...
                    Button(action: {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            PostRowActionsView(post: post)
                                .environment(
                                    postDataControllerProvider.get(for: post, client: client))
                        }
                    }
                    .buttonStyle(.plain)
                }
...
extension PostListView {
    private func handleScrollOffset(_ offset: CGFloat) {
        ...
        if offset < -100 {  // User has scrolled up significantly
            feedPositionService.resetNewPostsCount()
        }

        // Save current visible posts
        if !visiblePosts.isEmpty {
            feedPositionService.updateScrollPosition(visiblePosts: visiblePosts)
        }
    }
}
